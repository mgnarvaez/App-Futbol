...
function MaterialesPage() {
  ...
  // 1. Consulta con Caché de 5 min para Historial de Materiales
  const { data: dataMat, isLoading: loadingMat, refetch: refetchMat } = useQuery({
    queryKey: ["materiales-sheet"],
    queryFn: () => obtenerMaterialesSheet(),
    staleTime: 1000 * 60 * 5,
    refetchOnWindowFocus: false,
  });

  // 2. Consulta con Caché de 5 min para Plantel Oficial
  ...
  const historial = dataMat?.historial || [];
  const plantel = plantelData || [];
  const loadingHistorial = loadingMat;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Invalida caché para recalcular e incorporar el nuevo préstamo
        await refetchMat();
      } else {
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 space-y-8 pb-20 sm:pb-8">
      ...
      {error && (
        <div className="bg-destructive/15 border-l-4 border-destructive p-4 text-destructive flex items-center justify-between rounded">
      ...
      {mensajeExito && (
        <div className="bg-emerald-500/15 border-l-4 border-emerald-500 p-4 text-emerald-600 dark:text-emerald-400 flex items-center justify-between rounded">
          <div className="flex items-center gap-2">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full px-3 py-2 border border-input bg-background text-foreground rounded-lg focus:ring-2 focus:ring-green-500 focus:outline-none placeholder:text-muted-foreground"
              />

              {/* Menú Desplegable de Sugerencias */}
              {mostrarSugerencias && jugadoresFiltrados.length > 0 && (
                <div className="absolute z-20 left-0 right-0 top-full mt-1 bg-popover border border-border rounded-lg shadow-lg max-h-48 overflow-y-auto divide-y divide-border">
                  {jugadoresFiltrados.map((p, idx) => (
                    <button
                      key={idx}
                      type="button"
                      onClick={() => seleccionarJugador(p)}
                      className="w-full text-left px-3 py-2 hover:bg-accent text-sm flex justify-between items-center text-popover-foreground"
                    >
                      <div>
                  ...
                  <label
                    ...
                    className={`flex items-center justify-between p-2.5 rounded-lg border cursor-pointer transition-colors ${
                      materialesSeleccionados.includes(mat.id)
                        ? "bg-green-500/15 border-green-500/50 font-medium text-foreground"
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="pl-9 pr-4 py-1.5 text-sm border border-input bg-background text-foreground rounded-lg focus:ring-2 focus:ring-green-500 focus:outline-none w-full sm:w-60 placeholder:text-muted-foreground"
              />
            </div>
          </div>
            ...
            <div>
                  ...
                  <tbody className="divide-y divide-border">
                    {historialFiltrado.map((item) => (
                      <tr key={item.id} className="hover:bg-muted/30">
                        ...
                        <td className="py-3 px-3 text-foreground/80">{item.materiales}</td>
              ...
              <div className="sm:hidden space-y-3">
                {historialFiltrado.map((item) => (
                  <div
                    key={item.id}
                    className="p-4 border border-border rounded-lg bg-muted/20 space-y-2"
                  >
