  ...
  head: () => ({
    meta: [
      { title: "Panel de convocatorias de fútbol" },
      {
        name: "description",
        content:
          "Panel del día: inscripción por Google Form, suspensión por lluvia, gestión de bajas y control de pagos.",
      },
    ],
  }),
  component: Panel,
...
function Panel() {
  ...
  const { data: inscriptosSheet, refetch: refetchSheet } = useQuery({
    ...
    staleTime: 1000 * 60 * 5, // 5 minutos de caché para respuesta instantánea
    refetchOnWindowFocus: false, // Deshabilita recarga automática al volver a la ventana
  });
