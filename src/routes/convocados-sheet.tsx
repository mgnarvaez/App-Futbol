...
function ConvocadosSheetPage() {
  ...
  const { data: inscriptosSheet, isLoading, refetch } = useQuery({
    queryKey: ["inscriptos-sheet-solapas"],
    queryFn: () => obtenerInscriptosSheet(),
    staleTime: 1000 * 60 * 5, // 5 minutos de caché en memoria para velocidad instantánea
    refetchOnWindowFocus: false, // Evita re-consultar a Google al cambiar de ventana
  });
  ...
  // Agrupar los convocados por la sede/turno asignada en la Sheet
  const canton = (inscriptosSheet || []).filter(
    (i) => (i.sede || i.turno || "").toUpperCase().includes("CANTON"),
  );
  const sm = (inscriptosSheet || []).filter(
    (i) => (i.sede || i.turno || "").toUpperCase().includes("SM") || (i.sede || i.turno || "").toUpperCase().includes("MATIAS"),
  );
  const puertos = (inscriptosSheet || []).filter(
    (i) => (i.sede || i.turno || "").toUpperCase().includes("PUERTOS"),
  );
