...
export async function leerInscriptos(): Promise<InscriptoSheet[]> {
  try {
    ...
    const parseInscripto = (p: any, isVip: boolean): InscriptoSheet | null => {
      ...
      const rawPrefStr = (p.rawPref || "").toString().trim();
      const turnoStr = (p.turno || "").toString().trim();
      const prefStr = (p.pref || "").toString().trim();
      const sedeStr = (p.sede || "").toString().trim();

      const sede =
        detectarSede(rawPrefStr) ||
        detectarSede(turnoStr) ||
        detectarSede(prefStr) ||
        detectarSede(sedeStr);

      let pref = rawPrefStr;
      if (!pref || pref.toUpperCase().includes("CANCELADA")) {
        pref =
          [turnoStr, prefStr, sedeStr].find(
            (s) => s && !s.toUpperCase().includes("CANCELADA")
          ) || rawPrefStr || "CANTON";
      }
    ...
    // 1. Inscriptos VIP
    const vipPlayers =
      solapas.respuestas_vip?.players || solapas["Ingresos VIP"]?.players || [];
    for (const p of vipPlayers) {
      const item = parseInscripto(p, true);
      if (item) filas.push(item);
    }

    // 2. Inscriptos General
    const genPlayers =
      solapas.respuestas_4?.players || solapas["Ingresos General"]?.players || [];
    for (const p of genPlayers) {
      const item = parseInscripto(p, false);
      if (item) filas.push(item);
    }
