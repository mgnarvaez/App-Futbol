...
export async function leerInscriptos(): Promise<InscriptoSheet[]> {
  try {
    ...
    // Mapas de enriquecimiento desde los formularios VIP y General
    const vipMap = new Map<string, any>();
    const vipPlayersRaw = solapas.respuestas_vip?.players || solapas["Ingresos VIP"]?.players || [];
    for (const p of vipPlayersRaw) {
      const apodo = (p.apodo || p.rawNombre || "").toString().trim().toLowerCase();
      const email = (p.email || "").toString().trim().toLowerCase();
      if (apodo) vipMap.set(apodo, p);
      if (email) vipMap.set(email, p);
    }

    const formMap = new Map<string, any>();
    const genPlayersRaw = solapas.respuestas_4?.players || solapas["Ingresos General"]?.players || [];
    for (const p of genPlayersRaw) {
      const apodo = (p.apodo || p.rawNombre || "").toString().trim().toLowerCase();
      const email = (p.email || "").toString().trim().toLowerCase();
      if (apodo) formMap.set(apodo, p);
      if (email) formMap.set(email, p);
    }

    // Leemos los jugadores directamente desde las pestañas resultantes de la planilla
    const cantonPlayers = solapas.canton?.players || [];
    const puertosPlayers = solapas.puertos?.players || [];
    const smPlayers = solapas.SM?.players || solapas.sm?.players || [];

    const haySolapasOrganizadas =
      cantonPlayers.length > 0 || puertosPlayers.length > 0 || smPlayers.length > 0;

    if (haySolapasOrganizadas) {
      // 1. Canton
      for (const p of cantonPlayers) {
        const apodo = (p.apodo || "").toString().trim();
        const email = (p.email || "").toString().toLowerCase().trim();
        const key = apodo.toLowerCase() || email;

        if ((apodo || email) && !bajasSet.has(apodo.toLowerCase())) {
          const infoVip = vipMap.get(key);
          const infoForm = formMap.get(key);
          const isVip = Boolean(infoVip);
          const flex = Boolean(infoVip?.rawFlex || infoForm?.rawFlex);

          filas.push({
            timestamp: infoVip?.rawTimestamp || infoForm?.rawTimestamp || "",
            fecha: "",
            hora: "",
            email,
            turno: "20 hs CANTON",
            ...
            flexible: flex,
            apodo,
            juega_con_lluvia: Boolean(infoVip?.rawPlayIfRains || infoForm?.rawPlayIfRains),
            vip: isVip,
          });
        }
      }
    ...
    // Fallback: Si no se corrió la organización en las pestañas todavía, lee el formulario en bruto
    for (const p of vipPlayersRaw) {
      const email = (p.email || "").toString().toLowerCase().trim();
