...
export async function leerInscriptos(): Promise<InscriptoSheet[]> {
  try {
    ...
    // Mapeo auxiliar de VIP y Flexibilidad desde los datos raw del formulario
    ...
    for (const p of vipPlayersRaw) {
      const apodo = (p.apodo || p.rawNombre || "").toString().toLowerCase().trim();
      const email = (p.email || "").toString().toLowerCase().trim();
      if (apodo) vipSet.add(apodo);
      if (email) vipSet.add(email);
      if (p.rawFlex) {
        if (apodo) flexSet.add(apodo);
        if (email) flexSet.add(email);
      }
    }
    ...
    // Revisamos si las solapas calculadas por el algoritmo de Google ("canton", "puertos", "SM") tienen jugadores
    ...
    const tienenSolapasCalculadas =
      cantonPlayers.length > 0 || puertosPlayers.length > 0 || smPlayers.length > 0;

    if (tienenSolapasCalculadas) {
      // 1. Leer solapa CANTON
      for (const p of cantonPlayers) {
        const apodo = (p.apodo || "").toString().trim();
        const email = (p.email || "").toString().toLowerCase().trim();
        if ((apodo || email) && !bajasSet.has(apodo.toLowerCase())) {
          const isVip = vipSet.has(apodo.toLowerCase()) || vipSet.has(email);
          const isFlex = flexSet.has(apodo.toLowerCase()) || flexSet.has(email);
          filas.push({
            timestamp: "",
            fecha: "",
            hora: "",
    ...
    // FALLBACK: Si no hay solapas calculadas aún (antes de presionar "Convocar"), leemos las respuestas del formulario
    for (const p of vipPlayersRaw) {
      const email = (p.email || "").toString().toLowerCase().trim();
  ...
  try {
    ...
    if (data.success || data.status === "success") {
      ...
      return {
        ok: false,
        mensaje:
          data.error || data.message || "Error al registrar la baja en la planilla.",
      };
    }
  } catch (error) {
