...
export async function leerInscriptos(): Promise<InscriptoSheet[]> {
  try {
    ...
    // Mapeo auxiliar de VIPs y Flexibles según los formularios
    ...
    // 1. REVISAR SI EXISTEN SOLAPAS YA ORGANIZADAS POR EL SCRIPT ("20 hs CANTON", "21:15 hs PUERTOS", "20:00 hs SM")
    ...
    const tieneSolapasOrganizadas =
      cantonPlayers.length > 0 || puertosPlayers.length > 0 || smPlayers.length > 0;

    if (tieneSolapasOrganizadas) {
      // Leemos directamente los convocados resultantes del algoritmo del script
      
      // CANTON
      for (const p of cantonPlayers) {
        const apodo = (p.apodo || "").toString().trim();
        const email = (p.email || "").toString().toLowerCase().trim();
        if (!apodo && !email) continue;
        if (bajasSet.has(apodo.toLowerCase())) continue;
    ...
    // 2. FALLBACK SI AÚN NO SE EJECUTÓ EL SCRIPT DE CONVOCADOS (RESPUESTAS CRUDAS DEL FORMULARIO)
    for (const p of vipRaw) {
      const email = (p.email || "").toString().toLowerCase().trim();
      ...
      if ((apodo || email) && !bajasSet.has(apodo.toLowerCase())) {
        ...
        if (pref.toUpperCase().includes("CANCELADA") && (p.pref || p.sede)) {
          pref = (p.pref || p.sede).toString().trim();
        }
